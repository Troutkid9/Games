...
function playGame(playerInputValue, playerInputOpponent) {
  ...
  for (var i = 0; i < dealer.players.length; i++) {
    ...
    if (canTakeTurn === 0) {
      ...
      // [1] = index of a player
      var cardAndPlayerInt = playerTakingTurn.callCardAndPlayer(
    ...
    for (var i = 0; i < winnersArray.length; i++) {
      if (dealer.players[winnersArray[i]].id > 1) {
        $("#cardContainer").append(
          "<h3>OPPONENT " +
            (dealer.players[winnersArray[i]].id - 1) +
            " WINS!</h3>"
        );
      } else {
...
function instigateCall(cardToFind, playerToFish, playerTakingTurn) {
  ...
  if (!playerTakingTurn.human) {
    ...
    if (matchingValueIndicies < 1) {
      $(".actionExplain")
        .eq(playerTakingTurn.id - 2)
        .html("Hunted " + cardToFind + " from " + num + ".<br>GO FISH!");
    } else {
      $(".actionExplain")
        .eq(playerTakingTurn.id - 2)
        .html(
    ...
    if (matchingValueIndicies < 1) {
      $("#playerInfo").html(
        "Hunted " + cardToFind + " from " + num + ". GO FISH!"
      );
    } else {
      $("#playerInfo").html(
        "Hunted " +
...
class Dealer {
  ...
  dealHands() {
    ...
    if (
      ...
      this.playerCount < this.DEAL_THRESHOLD
    ) {
      toDeal = 7;
    } else if (
      this.playerCount >= this.DEAL_THRESHOLD &&
...
class Player {
  ...
  updateSetsUI() {
    ...
    if (this.human) {
      ...
      $("#opponentContainer")
        .children("[number=" + (this.id - 1) + "]")
        .eq(0)
        .children(".opponentSets")
        .eq(0)
        .html(setsString);
    }
  }
