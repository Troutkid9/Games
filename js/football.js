...
function setInitialDelay() {
  setTimeout(function () {
    ...
    updateStatus('You are team <br> in <span style="color:blue">blue</span>');
  }, 1500);
}
...
function startTimer(duration) {
  ...
  countdown = setInterval(function () {
    ...
    if (--timer < 0) {
      ...
      if (homeScore > awayScore) updateStatus("GAME OVER!<br>Liverpool Won!");
      else if (awayScore > homeScore)
        updateStatus("GAME OVER!<br>Juventus Won!");
      else updateStatus("GAME OVER!<br>Draw!");
    }
  }, 1000);
}
...
function drawAwayDefenders() {
  ...
  if (x > lcbX && lcbX < (paddleX * 3) / 4) j += aiSpeed;
  else if (lcbX > (paddleX * 1) / 4) j -= aiSpeed;
  if (x > rcbX && rcbX < (paddleX * 3) / 4) j += aiSpeed;
