require(["ramda", "webgl_helpers", "functional_utils"], function (r, w, fun) {
  ...
  var holeDepth = 0.05;
  var holePattern = [
    [-holeFlatWidth - halfHoleWidth, 0],
    [-halfHoleWidth, 0],
    [-halfHoleWidth, -holeDepth],
    [halfHoleWidth, -holeDepth],
    [halfHoleWidth, 0],
    [holeFlatWidth + halfHoleWidth, 0],
  ];
  ...
  var landscape = function () {
    var points = insertFlatSegments(
      flatChance,
      fun.cons([0, 0.4], fun.repeatedly(1 / minSegLen + 1, randomPoint))
    );
  ...
  var drawBall = function () {
    ...
    var ballPoints = r.map(
      pointOnCircle,
      r.map(function (factor) {
        return ((2 * Math.PI) / ballSectors) * factor;
  ...
  var drawFlag = function () {
    ...
    drawGraphics(r.flatten(flag), gl.TRIANGLES, [1, 0, 0], transformation);
  };
  ...
  var drawExplosion = function (exp) {
    ...
    drawGraphics(
      r.flatten(r.map(pos, exp.particles)),
      gl.POINTS,
      [1, 1, 0],
      transformation
    );
  ...
  var setupHole = function () {
    ...
    var pointsNeeded =
      r.length(
        r.takeWhile(function (p) {
          return p[0] < 1;
        }, land)
      ) + 1;
    land = r.take(pointsNeeded, land);

    startingPosition = ballPosition = fun.updateNumber(
      1,
      0.001,
  ...
  var bounce = function (delta) {
    ...
    var calculateVelocity = function (velocity) {
      ...
      if (
        magnitude(velocity) < velocityThreshold &&
  ...
  var createParticle = function () {
    ...
    return {
      position: scaleVec(0.01, [rand1(), rand1()]),
  ...
  var spawnTrails = function (particles) {
    var copy = function (p) {
      return { position: r.map(r.identity, p.position), velocity: [0, 0] };
    };
  ...
  var celebrate = function () {
    celebrating = true;
    var duration =
      (numExplosions - 1) * explosionInterval + explosionLife + newHoleDelay;
    ...
    var runCelebration = function (now) {
      ...
      if (celebrationTime > explosionInterval * spawnedExplosions) {
        var pos = [0.7 + rand(-0.2, 0.2), 0.8 + rand(-0.1, 0.1)];
  ...
  var gaussianTexture = function (size) {
    ...
    for (var x = 0; x < size; x++) {
      for (var y = 0; y < size; y++) {
        result.push(
          color(gaussian(magnitude(vecSub([x, y], mid)) / distToEdge))
        );
      }
    }
    return result;
